...
class GUI():
    ...
    def TrainModel(self):
        ...
        self.model.fit(images,labels,epochs=25,batch_size=10)
    ...
    def Spectrogram(self):
        # Spectrogram code was not made by me but by a helpful stack overflow user
        wav_file = tf.placeholder(tf.string)
        audio_binary = tf.read_file(wav_file)
        waveform = audio_ops.decode_wav(audio_binary, desired_channels=1)
        spectrogram = audio_ops.audio_spectrogram(waveform.audio,window_size=1024,stride=64)
    ...
    def check(self):
        ...
        print(self.model.predict_classes(np.array(image_processor).reshape(1,256,256,1)))
        print(self.model.predict_proba(np.array(image_processor).reshape(1,256,256,1)))
        guess = int(self.model.predict_classes(np.array(image_processor).reshape(1,256,256,1))[0])
        proba = float(self.model.predict_proba(np.array(image_processor).reshape(1,256,256,1))[0][guess])
        
        if guess == 0:
            self.Update_output('Password accepted with probability of '+ str(proba*100)+' %')
        else:
